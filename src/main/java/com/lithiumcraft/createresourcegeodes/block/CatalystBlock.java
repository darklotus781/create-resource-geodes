...
abstract public class CatalystBlock extends Block {
    ...
    public InteractionResult use(BlockState state, Level world, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
        ...
        if (!world.isClientSide() && held.getItem() == REDSTONE.asItem()) {
            if (!player.getAbilities().instabuild)
                held.shrink(1); // first shrink, becasue we don't want to delay on that
            ...
            for (int x = -RADIUS; x <= RADIUS; x++) {
                for (int y = -RADIUS; y <= RADIUS; y++) {
                    for (int z = -RADIUS; z <= RADIUS; z++) {
                        ...
                        if (x == 0 && y == 0 && z == 0 || !world.getBlockState(temp).toString().contains("minecraft:air"))
                            continue;  // not sure what did this line do, so I replaced it with central block check.
                        // if (!world.getBlockState(temp).is(Blocks.AIR)) continue; // uncomment if you only want to convert AIR blocks.
                        if (random <= CHANCE) // 23 works, 71 doesn't (if CHANCE = 50)
                        {
    ...
    @SubscribeEvent
    public static void tickEvent(TickEvent.LevelTickEvent w) {
        ArrayList<BlockPos> removeMe = new ArrayList<>();
        schedule.forEach(((blockPos, tuple) -> {
            if (!w.level.equals(tuple.getB())) {
            ...
            if (tuple.getA() <= 0) {
                ...
                tuple.getB().setBlock(blockPos, GeodeShit.Veridium.defaultBlockState(), 3); // Replace this with something
                return;
            }
